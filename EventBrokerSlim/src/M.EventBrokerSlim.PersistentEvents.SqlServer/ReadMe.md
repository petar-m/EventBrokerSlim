...
SQL Server storage backend for [EventBrokerSlim](https://github.com/petar-m/EventBrokerSlim/blob/main/EventBrokerSlim/ReadMe.md) persistent events - durable, at-least-once event delivery that survives process restarts.
...
The package requires a schema, table, and indexes in your SQL Server database. There are two ways to set them up:
...
{
    ConnectionString = "Server=localhost;Database=mydb;User Id=myuser;Password=mypassword;TrustServerCertificate=True",
    Schema = "ebs_0",
    Table = "events"
};
...
The call is idempotent - it checks for existing objects before creating and is safe to run on every startup in development.

> [!WARNING]
> `CreateEventsTable()` requires DDL permissions (create schemas, tables, and indexes). Production applications should typically run this as part of a deployment or migration process, not at application startup.

### Option 2: Manual SQL script

Apply the `initialize_db.sql` script included in the package source. The default schema name is `ebs_0` and the default table name is `events` - replace them with unique names if running multiple event broker instances against the same database.
...
### 4. Configure the event broker with SQL Server persistence
...
serviceCollection.AddEventBroker(x => x
    ...
    {
        db.ConnectionString = "Server=localhost;Database=mydb;User Id=myuser;Password=mypassword;TrustServerCertificate=True";
        db.Schema = "ebs_0";
        db.Table = "events";
...
| `Table` | `"events"` | Name of the events table. Use a unique table name per event broker instance when sharing the same schema. |
...
**Not a transactional outbox.** The event write to SQL Server is not atomic with the caller's own database transaction.
...
The package creates the following schema (default name `ebs_0`) and table (default name `events`):

### Events Table

| Column | Type | Description |
|---|---|---|
| `id` | `BIGINT` (PK, Identity) | Auto-incrementing record identifier |
| `event_id` | `NVARCHAR(128)` | Unique identifier for the event instance (shared across fan-out records) |
| `event_name` | `NVARCHAR(128)` | Registered event name from `EventRegistry` |
| `handler_name` | `NVARCHAR(128)` | Registered handler name |
| `payload` | `NVARCHAR(MAX)` | JSON-serialized event data |
| `status` | `INT` | 1=Scheduled, 2=InProgress, 3=Completed, 4=DeadLettered |
| `scheduled_at` | `DATETIMEOFFSET` | When the record becomes eligible for processing |
| `retry_attempt_count` | `INT` | Number of retry attempts |
| `retry_last_delay` | `BIGINT` | Duration of the last retry delay (milliseconds) |
| `claimed_at` | `DATETIMEOFFSET` | When the record was claimed for processing |
| `created_at` | `DATETIMEOFFSET` | When the record was created |
| `last_updated_at` | `DATETIMEOFFSET` | When the record was last updated (used for optimistic concurrency) |
| `last_error` | `NVARCHAR(MAX)` | Error message from the most recent failure |
...
| Index | Purpose |
|---|---|
| **Polling** (filtered, `status=1`) | Covers the high-frequency polling query. Includes `id`, `last_updated_at`, `event_name`, `handler_name` for covering index scans. |
| **Timeout** (filtered, `status=2`) | Enables efficient timeout detection for in-progress records. |
| **Cleanup** (filtered, `status IN (3,4)`) | Supports retention-based deletion of completed and dead-lettered records. |
