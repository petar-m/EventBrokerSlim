...
// Register EventBroker with SQL Server persistence
builder.Services.AddEventBroker(b => b
    .WithMaxConcurrentHandlers(4)
    .WithSqlServerPersistence((db, broker) =>
    {
        db.ConnectionString = 
...
builder.Services.AddEventHandlerPipeline<SampleEvent>(pipeline, opt => opt.WithHandlerName("sample-event-handler"));
