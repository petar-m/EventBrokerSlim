...
public class RescheduleTest : IDisposable
{
    ...
    public async Task Event_exceeding_processing_timeout_is_rescheduled()
    {
        ...
        await AssertStatusAsync(receivedEvent.EventRecord.Id, EventStatus.Scheduled);
    }

    private async Task AssertStatusAsync(string id, EventStatus expectedStatus)
    {
        var collection = _setup.MongoClient.GetDatabase(Setup.DatabaseName).GetCollection<BsonDocument>(nameof(RescheduleTest));
