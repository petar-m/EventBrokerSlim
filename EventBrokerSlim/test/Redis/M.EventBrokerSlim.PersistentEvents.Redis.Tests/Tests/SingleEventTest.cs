...
public class SingleEventTest : IDisposable
{
    ...
    public SingleEventTest(Setup setup)
    {
        ...
        var services = new ServiceCollection()
            .AddEventBroker(x => x.WithRedisPersistence((db, cfg) => db.ConnectionString = setup.ConnectionString))
    ...
    public async Task Event_sent_and_received()
    {
        ...
        var sampleEvent = new SampleEvent("hello from handler!");
        ...
        await Assert.StatusIsAsync(EventStatus.Completed, receivedEvents[0].EventRecord.Id, _setup.DataSource.GetDatabase());
    }
