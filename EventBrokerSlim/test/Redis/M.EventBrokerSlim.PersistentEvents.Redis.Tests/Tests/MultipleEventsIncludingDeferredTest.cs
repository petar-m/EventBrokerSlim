...
public class MultipleEventsIncludingDeferredTest : IDisposable
{
    ...
    public async Task Event_sent_and_received()
    {
        ...
        await broker.PublishDeferred(event2, TimeSpan.FromSeconds(2));
        await broker.PublishDeferred(event3, TimeSpan.FromSeconds(2));
        ...
        Assert.True(receivedEvent1.ReceivedAt < receivedEvent2.ReceivedAt);
        Assert.True(receivedEvent1.ReceivedAt < receivedEvent3.ReceivedAt);
        Assert.True(receivedEvent4.ReceivedAt < receivedEvent2.ReceivedAt);
        Assert.True(receivedEvent4.ReceivedAt < receivedEvent3.ReceivedAt);

        var db = _setup.DataSource.GetDatabase();
        await Assert.StatusIsAsync(EventStatus.Completed, receivedEvent1.EventRecord.Id, db);
        await Assert.StatusIsAsync(EventStatus.Completed, receivedEvent2.EventRecord.Id, db);
        await Assert.StatusIsAsync(EventStatus.Completed, receivedEvent3.EventRecord.Id, db);
        await Assert.StatusIsAsync(EventStatus.Completed, receivedEvent4.EventRecord.Id, db);
    }
