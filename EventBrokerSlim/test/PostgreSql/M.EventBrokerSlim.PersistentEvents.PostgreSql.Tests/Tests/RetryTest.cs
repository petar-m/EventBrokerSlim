...
public class RetryTest : IDisposable
{
    ...
    public async Task Event_retried_and_successfully_handled()
    {
        ...
        await receiver.WaitForEventsAsync(4, TimeSpan.FromSeconds(10000000), TestContext.Current.CancellationToken);
        ...
        Assert.Equal(sampleEvent, receivedEvent.Event);
        await AssertCompletedStatus(receivedEvent.EventRecord);
    }

    private async Task AssertCompletedStatus(EventRecord eventRecord)
    {
        using var connection = _setup.DataSource.CreateConnection();
        using var command = connection.CreateCommand();
        command.CommandText = $"SELECT status FROM ebs_5.{nameof(RetryTest)} WHERE id = @id";
        ...
        Assert.Equal(EventStatus.Completed, (EventStatus)status!);
    }
