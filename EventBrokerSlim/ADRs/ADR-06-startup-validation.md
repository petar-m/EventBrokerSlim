...
- Every event type in `EventRegistry` should have at least one handler with a `handlerName` registered — otherwise, events of that type will be written to the store but never claimed
